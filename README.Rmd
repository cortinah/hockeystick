...
The goal of `hockeystick` is to make essential Climate Change datasets easily available to non-climate experts. `hockeystick` users can download the latest raw data from authoritative sources as well as view it via pre-defined ggplot2 charts. Datasets include atmospheric CO<sub>2</sub>, instrumental and ice-core temperature records, sea levels, and Arctic sea-ice. Additional visualizations using this data will be added over time.
...
Retrieve NOAA/ESRL Mauna Loa CO<sub>2</sub> Observatory concentration and plot it:

```{r example, out.width='70%', fig.retina=2}
library(hockeystick)
ml_co2 <- get_carbon()
plot_carbon(ml_co2)
```
...
## Notes and Resources
- While all data is compiled from reliable sources, I am not a climate scientist. I am building `hockeystick` because of my interest in tracking climate data and making it available to the community.
