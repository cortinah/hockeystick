...
get_seaice <- function(pole='N', month=7, measure='extent',
  ...
  status <- tryCatch({  download.file(filename, dl, mode = 'wb') }, error = function(e) {TRUE}, warning = function(w) {TRUE} )
...
plot_seaice <- function(dataset = get_seaice(), title='Arctic Sea Ice', print=TRUE) {
  ...
  plot <-  ggplot(dataset, aes(x=date, y=.data[[colnames(dataset)[2]]])) +geom_line(linewidth=1, color='firebrick1') +
    scale_x_date(name=NULL, breaks='5 years', date_labels='%Y', limits=c(ymd('1978-01-01'), ceiling_date(max(dataset$date), 'years'))) +
    scale_y_continuous(n.breaks = 6, limits=c(0,max(dataset[,2]))) + geom_smooth(method='lm', se=FALSE, linetype=2, linewidth=0.5) + theme_bw(base_size = 12) +
    labs(title=title,
