...
get_fires_emissions <- function(place='WORLD', year=as.numeric(format(Sys.Date(), "%Y")), use_cache = TRUE,
  ...
  cumul <- cumul |> select(plt=plt, dt=dt, cum_curv=curv, cum_minv=minv, cum_maxv=maxv, cum_avgv=avgv)
...
#' Download and plot essential climate data
#'
#' Plots the wildfire greenhouse gas emissions data retrieved using `get_fires_emissions()` with ggplot2.
#' The output ggplot2 object may be further modified.
#'
#'
#' @name plot_fires_emissions
#' @param dataset Name of the tibble generated by `get_fires_emissions`
#' @param pollutant (string) Select which pollutant to plot. May be one of \code{"CO2"} (the default),
#'   \code{"BC"}, \code{"C"}, \code{"CH4"}, \code{"CO"}, \code{"NMHC"}, \code{"NOX"},
#'   \code{"OC"}, \code{"PM2.5"}, \code{"SO2"}, or \code{"TC"}.
#' @param style (string) Select cumulative or weekly view. May be \code{"cumulative"} (year-to-date totals,
#'   the default) or \code{"weekly"} (weekly values).
#' @param print (boolean) Display the ggplot2 chart, defaults to TRUE. Use FALSE to suppress display.
#'
#' @return Invisibly returns a ggplot2 object with the wildfire emissions chart
#'
#' @details `plot_fires_emissions` invisibly returns a ggplot2 object with a pre-defined wildfire
#' emissions chart using data from `get_fires_emissions`. By default the chart is also displayed.
#' The shaded ribbon shows the historical minimum-to-maximum range since 2003, and the blue line
#' shows the historical average. Emissions are expressed in millions of tons. Users may further
#' modify the output ggplot2 chart.
#'
#' @import ggplot2
#'
#' @examples
