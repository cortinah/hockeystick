...
unzip(dl, 'CSIRO_Recons_gmsl_mo_2011.csv', exdir = td, overwrite = TRUE)
gmsl_tide <- readr::read_csv(file.path(td,'CSIRO_Recons_gmsl_mo_2011.csv'), col_types = readr::cols(`GMSL uncertainty (mm)` = readr::col_skip()))
...
plot <-  ggplot(dataset, aes(x=date, color=method, y=gmsl)) +geom_line(size=1) + theme_bw() +
         ...
         labs(title='Sea Level Rise', subtitle='Tide gauges: 1880-2009; Satellite: 1992-present, calibrated to 1993 mean tide gauge.', y= 'Variation (mm)',
