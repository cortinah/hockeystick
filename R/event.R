...
fcst <- fcst |> select(date, y=extmin) |> tail(12*10)
...
fc |> filter(.model=='arima') |> autoplot(level = 66) + geom_hline(yintercept = 14.2) + scale_y_continuous(n.breaks = 10) + geom_hline(yintercept = 14)
fc |> filter(date==tsibble::make_yearmonth(2025,03)) |> hilo(level = 50)
